...
## 見本に学ぶ — 「方程式の難易度」デッキの設計原理

`方程式の難易度.html`（GitHub Pages の特別編）は、この講義でいちばん出来のよい教材である。
その作りを分解すると、次の 6 つが効いている。**スライドを直すときはこの順で当てる。**

1. **タイトルが結論を言い切る。**
   「線形代数方程式」ではなく「**Ax = b は、有限回で終わる唯一の階層**」。
   見出しを読むだけで主張が伝わるか、を basis にする。

2. **1 手ずつ出す（段階開示）。** 3 段の導出を 1 枚に詰めず、
   1 段目だけ → 2 段目まで → 3 段目まで、と 3 枚に分ける。まだ出していない段は
   薄く（ゴースト）残しておくと、「これから何段あるか」が同時に伝わる。
   `pws_eqfig.derivation_figure(..., reveal=i)` の `reveal` がこれをやる。

   ```python
   for i in (1, 2, 3):
       derivation_figure(f"chNN_deriv_xxx_{i}.png", steps=steps, result=result, reveal=i)
   ```
   スライド側は `figure-full` を 3 枚並べ、見出しは同じ、キャプションだけ変える。
   **文字だけの `sections` + `<!-- build -->` は、これに置き換える**（枚数は変わらない）。

3. **数式の記号と図の色を必ず対応させる。** 式の中の記号を色分けし、
   同じ色を図の中でも使う。読み手が目で追える線を 1 本作る。

4. **図は実計算から描く。** 概念図で終わらせず、その回の数値で描く。

5. **「何が変わったか」の縦串を通す。** 見本は「一段降りるたびに保証が一つ消える」で
   15 枚をつないでいる。各回にも、その回で何の前提が変わるのかを 1 文で置く。

6. **最後に結論の帯。** 図の下端に、その 1 枚で持ち帰るものを 1 文で書く。

適用の優先順位は、①文字だけの導出スライド → ②結論を言わない見出し → ③色の非対応。
